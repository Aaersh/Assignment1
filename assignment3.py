...
'''this is a program to count the number of occurrences of each word or character in the string entered by the user. (Count the Number of occurrences
of each character only if the single word is entered by the user)'''

#defining a function to count number of occurences of each word
def freq(str):
    str = str.split()                                                                 #splitting each word of the given string
    str2 = []                                                                         #creating a blank list
    for i in str:
        if i not in str2:
            str2.append(i)                                                            #adding each word to the blank list
    for i in range(0,len(str2)):
        print("The frequency of", str2[i], "is", str.count(str2[i]))                  #printing output message by using count function


user_str = input("Enter a string\n")                                                  #taking input from the user
length = len(user_str.split())                                                        #storing the number of words in length variable
if length>1:
    freq(user_str)                                                                    #function calling
else:
    charr = {i: user_str.count(i) for i in set(user_str)}                             # iterate over the set converted string and get the count of each character in original string
    print("The count of all characters in given string is :\n ", str(charr))
