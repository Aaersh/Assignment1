...
#****************************************************** QUESTION 6 *********************************************************
#program to make a meaningful word from given letters in a word
from itertools import permutations
import enchant
d = enchant.Dict("en_US")
op = set()

george = str(input("Enter the word\n"))
letter = [x.lower() for x in george]
for y in list(permutations(letter,len(george))):
    z = "".join(y)
    if len(z)>1:
        if d.check(z):
            op.add(z)
